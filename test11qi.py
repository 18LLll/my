...
import pickle #用来保存模型和output_uniques 变量

#读取数据集，并将字符编码指定为gbk，防止中文报错
penguin_df = pd.read_csv('（企鹅识别数据）penguins-chinese.csv',encoding='gbk')
#删除缺失值所在的行
...
#从features和output_codes 这两个数组中将数据集划分为训练集和测试集#训练集为80%，测试集为20%(1-80%)
#返回的x_train和 y_train为划分得到的训练集特征和标签
#x_test和y_test 为划分得到的测试集特征和标签
#这里标签和目标输出变量是一个意思
